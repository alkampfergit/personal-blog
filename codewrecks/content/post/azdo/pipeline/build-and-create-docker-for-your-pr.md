...
date: 2024-06-29T07:00:42+00:00
draft: true
...
The basic concept is: develop is a branch that should be **considered production** and it is not uncommon for teams to deploy develop branch automatically in internal production servers, a procedure called **dogfooding**. Here in Nebula Team we deploy in our internal production develop branch, and sometimes you intercept bug before they hit master and production of all customer.

> But the question is: how can bugs reach develop if we have Pull Requests?
...
***Lack of context or missing big picture**: in most situation peoples uses the web interface to look at changed file, they can spot problems, but quite often you need to examine the code in a real IDE and run in your local environment.
...
> Pull Request is for the team and for stakeholder, people should be able to run the software of the PR then approve or not.
...
Ok, from now we proceeded creating some DOCKERFILE to create docker images from the result of these build. Since we are working with .NET 8 it is quite simple. Then we created a nice powershell script that actually **download from a private Azure storage account some 7z files with backup of our demo, test or local instances**. These archives are generated automatically and capture the image of an environment (basically is a complete backup for disaster recovery).

In these test dockerfile we add instructions to verify if we mounted such archive in a standard location, if true, when the container starts, it will **unpack the archive and run a restore script that restore the environment**.
...
***Figure 4:*** *You can simply queue the pipeline that create release package and then trigger Docker image creation pipeline*
